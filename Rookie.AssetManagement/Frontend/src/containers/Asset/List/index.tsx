...
const ListAsset = () => {
    ...
    if (assetResult && (query.page == 1 || typeof (query.page) == 'object')) {
        const currentAsset = output.items.find(asset => asset.assetCode === assetResult.assetCode)
        if(currentAsset) {
            console.log("Yay, it founded");
            const index = output.items.indexOf(currentAsset);
            output.items.splice(index, 1);
        }
        output.items.reverse()
        output.items.push(assetResult)
        output.items.reverse()
    }
