...
import { CategoryOptions, StateOptions } from 'src/constants/selectOptions'
import { createAsset } from './reducer';
import axios from 'axios';

const state = {
    persons: []
}
    axios.get(`https://localhost:5001/api/Asset`)
      .then(res => {
...
const AssetFormContainer : React.FC<Props> = ({ initialAssetForm  = { ...initialFormValues } }) => {
    ...
    const isUpdate = initialAssetForm.assetId ? true : false;
            ...
            onSubmit = {(values) => {
                ...
                setTimeout(() => {
                    if (isUpdate) {
                        ...
                        // generateDateRequest(values.joinedDate, values.dateOfBirth)
